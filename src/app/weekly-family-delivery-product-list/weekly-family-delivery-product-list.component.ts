...
export class WeeklyFamilyDeliveryList {
  ...
  updateDelivery() {
    ...
    let cols: ColumnSetting<any>[] = [d.assignedHelper.getColumn(this.dialog, h => h.volunteer.isEqualTo(true))];
    if (d.status.value == WeeklyFamilyDeliveryStatus.Delivered) {
      cols.push(dc),
        cols.push(d.deliveredBy.getColumn(this.dialog, h => h.volunteer.isEqualTo(true)));
    }
